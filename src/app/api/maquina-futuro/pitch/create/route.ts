...
import { generateInvestorPitch } from "@/lib/ai/maquina-futuro";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: vision } = await supabase
      ...
      .select("title, problem_statement, solution_statement, final_scene, target_users")
      .eq("id", visionId)
      .eq("owner_id", user.id)
      .maybeSingle();
    ...
    const details = [
      ...
      vision.target_users && `Usuarios: ${vision.target_users}`
    ].filter(Boolean).join("\n");

    const output = await generateInvestorPitch(vision.title, details);
    ...
    console.error("Pitch create error:", err);
