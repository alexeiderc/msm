...
import { generateMeditation } from "@/lib/ai/maquina-futuro";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: vision } = await supabase
      ...
      .select("title, problem_statement, solution_statement, final_scene, divine_purpose")
      .eq("id", visionId)
      .eq("owner_id", user.id)
      .maybeSingle();
    ...
    const details = [
      ...
      vision.divine_purpose && `Proposito: ${vision.divine_purpose}`
    ].filter(Boolean).join("\n");

    const output = await generateMeditation(vision.title, details);
    ...
    console.error("Meditation create error:", err);
