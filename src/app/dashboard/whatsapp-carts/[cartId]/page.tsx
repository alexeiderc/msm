...
import { PackageCheck, Truck, Clock, XCircle } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { getWhatsAppCart } from "@/server/actions/whatsapp-cart";

export const dynamic = "force-dynamic";

const statusConfig: Record<string, { label: string; color: string; icon: typeof PackageCheck }> = {
  enviado: { label: "Enviado", color: "border-blue-200 bg-blue-50 text-blue-700", icon: Clock },
  procesando: { label: "Procesando", color: "border-amber-200 bg-amber-50 text-amber-700", icon: Truck },
  completado: { label: "Completado", color: "border-green-200 bg-green-50 text-green-700", icon: PackageCheck },
  cancelado: { label: "Cancelado", color: "border-red-200 bg-red-50 text-red-700", icon: XCircle },
};

export default async function WhatsAppCartDetailPage({ params }: { params: Promise<{ cartId: string }> }) {
  ...
  return (
    <section className="mx-auto max-w-3xl px-4 py-8 pb-24">
        ...
        <p className="mt-2 text-slate-600">
          Enviado el {new Date(row.sent_at).toLocaleDateString("es-CU", {
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          <div className="rounded-lg border border-msm-line bg-white p-4 shadow-soft">
            ...
            <p className="mt-2 text-3xl font-bold capitalize">{row.status}</p>
            {row.tracking_code && (
          ...
          <div className="rounded-lg border border-msm-line bg-white p-4 shadow-soft">
            ...
            <div className="mt-3 space-y-1 text-sm">
              <p><span className="font-semibold">Nombre:</span> {row.customer_name}</p>
              <p><span className="font-semibold">Teléfono:</span> {row.customer_phone}</p>
              {row.customer_email && <p><span className="font-semibold">Email:</span> {row.customer_email}</p>}
            </div>
          </div>
          ...
          <div className="rounded-lg border border-msm-line bg-white p-4 shadow-soft">
            ...
            <div className="mt-3 space-y-1 text-sm">
              ...
              <p>{row.delivery_municipality}, {row.delivery_province}</p>
              {row.beneficiary_name && (
                <>
                  <p className="mt-2 font-semibold">Beneficiario:</p>
                  <p>{row.beneficiary_name}</p>
                  {row.beneficiary_phone && <p>{row.beneficiary_phone}</p>}
                </>
              )}
            </div>
          </div>
